...
def query():
  ...
  if restaurant_query:
    ...
    if restaurant_query in restaurant_to_index.keys():
      top_restaurants = get_top(restaurant_query, price_query, cuisine_query, ambiances_query, 3, review_weight, ambiance_weight, False, None)
      app.logger.critical("got restaurants")
      output_message = "Your search: " + restaurant_query
      data = web_scraping(top_restaurants, restaurant_to_index[restaurant_query], False, None)
