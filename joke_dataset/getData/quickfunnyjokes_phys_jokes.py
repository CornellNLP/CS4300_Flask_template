...
def extract_jokes():
	result = []
	url = 'https://www.quickfunnyjokes.com/physics.html'
	...
	for joke in sel(html_elmnts):
		...
		while i < len(lst):
			...
			if len(tmp)!= 0:
				if 'Q:' in tmp:
					result.append({'joke': tmp + '\n ' + (lst[i+1].strip()), 'score': None, 'categories': ['Physics']})
					i += 1
				else:
					result.append({'joke': tmp, 'score': None, 'categories': ['Physics']})
			i += 1
...
try:
	...
	with open('../json/raw/quickfunnyjokes_phys_jokes.json', 'w') as file:
