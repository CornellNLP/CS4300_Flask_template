...
from app.irsystem.models.shared_variables import file_path, file_path_name
...
def make_description_query(subreddit):
    return "http://www.reddit.com/r/" + subreddit + "/about.json"
...
def create_description_dataset():
    with open(reddit_list) as csvfile:
        ...
        for subreddit_row in reader:
            ...
            print("Querying: " + make_description_query(subreddit))
            # define user-agent in headers to avoid Error 429- too many requests
            raw_response = requests.get(make_description_query(subreddit), headers={
                'User-agent': 'x'})
            response = raw_response.json()
            desc = response['data']['public_description']
            print(desc)
            dataset[subreddit] = desc
        filename = "{}-descriptions.pickle".format(file_path_name)
...
def create_dataset():
    with open(reddit_list) as csvfile:
        ...
        for subreddit_row in reader:
            ...
            print(str(subreddit_counter) + " query: " +
                  str(make_query(subreddit)), "\n")
        ...
        for subreddit_name, freq in cnt.most_common():
            if freq < num_posts * 0.20:
                ...
                print("removing " + subreddit_name +
                      " with a low count " + str(freq))
